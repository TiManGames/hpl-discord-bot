...
4. Claude reads the relevant docs on its own (via a sandboxed `read_file` tool) and posts the answer in the thread.
...
src/
  ...
  tools.ts            # Sandboxed read_file tool + docs file-tree manifest builder
...
The agent loop honours SAP's `x-retry-after` header on 429s (the SDK's built-in backoff ignores it), and if it hits the tool-step cap mid-research it forces a final answer from what it has already read.
