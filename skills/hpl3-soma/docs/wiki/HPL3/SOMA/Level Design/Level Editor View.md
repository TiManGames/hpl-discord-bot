...
'''Linux Note:'''  Left Super (aka "Windows" or Apple key) and the Z key are also the view key as the Alt key is usually bound by the Window Manager. <br />'''Mac<nowiki> OS</nowiki> X Note:'''  A three button mouse is required to use the editor.
